...
if [[ "$ENV_REDIS_DIR" ]]; then
    sed -i "s#dir ./#dir ${ENV_REDIS_DIR}#g" ${ENV_REDIS_CONF}
fi

if [[ "$ENV_REDIS_DBFILENAME" ]]; then
...
if [[ "$ENV_REDIS_PORT" ]]; then
    sed -i "s/port 6379/port ${ENV_REDIS_PORT}/g" ${ENV_REDIS_CONF}
fi

sed -i "s/# requirepass foobared/requirepass root/g" ${ENV_REDIS_CONF}
sed -i "s/# masterauth <master-password>/masterauth root/g" ${ENV_REDIS_CONF}
