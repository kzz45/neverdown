...
type MysqlLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Mysql, err error)
...
// List lists all Mysqls in the indexer.
func (s *mysqlLister) List(selector labels.Selector) (ret []*v1.Mysql, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Mysql))
	})
	return ret, err
}
...
type MysqlNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Mysql, error)
...
// List lists all Mysqls in the indexer for a given namespace.
func (s mysqlNamespaceLister) List(selector labels.Selector) (ret []*v1.Mysql, err error) {
...
// Get retrieves the Mysql from the indexer for a given namespace and name.
func (s mysqlNamespaceLister) Get(name string) (*v1.Mysql, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("mysql"), name)
	}
	return obj.(*v1.Mysql), nil
}
