...
type AccessControlLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AccessControl, err error)
...
// List lists all AccessControls in the indexer.
func (s *accessControlLister) List(selector labels.Selector) (ret []*v1.AccessControl, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AccessControl))
	})
	return ret, err
}
...
type AccessControlNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AccessControl, error)
...
// List lists all AccessControls in the indexer for a given namespace.
func (s accessControlNamespaceLister) List(selector labels.Selector) (ret []*v1.AccessControl, err error) {
...
// Get retrieves the AccessControl from the indexer for a given namespace and name.
func (s accessControlNamespaceLister) Get(name string) (*v1.AccessControl, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("accesscontrol"), name)
	}
	return obj.(*v1.AccessControl), nil
}
