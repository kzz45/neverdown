...
func (mc *MysqlController) newService(m *openxv1.Mysql, in openxv1.MysqlConfig) (*corev1.Service, error) {
	...
	annotations, err := mc.loadbalancer.Annotations(in.Service.Spec.Type, in.CloudNetworkConfig, m.Namespace)
	if err != nil {
		return nil, err
	}
...
func (mc *MysqlController) syncServices(m *openxv1.Mysql, svcList []*corev1.Service) error {
	...
	if m.Spec.Master.Service != nil && len(m.Spec.Master.Service.Spec.Ports) > 0 {
		...
		a, err := mc.newService(m, m.Spec.Master)
		if err != nil {
			return err
		}
		cur = append(cur, a)
	...
	if m.Spec.Slave.Service != nil && len(m.Spec.Slave.Service.Spec.Ports) > 0 {
		...
		a, err := mc.newService(m, m.Spec.Slave)
		if err != nil {
			return err
		}
		cur = append(cur, a)
	...
	for _, v := range add {
		if _, err := mc.svcControl.Create(v); err != nil {
			errlist = append(errlist, err)
		}
	}
	for _, v := range update {
		if _, err := mc.svcControl.Update(v); err != nil {
			errlist = append(errlist, err)
		}
	}
	for _, v := range del {
		if err := mc.svcControl.Delete(v); err != nil {
			errlist = append(errlist, err)
		}
	}
