...
func mysqlServerId(role openxv1.ClusterRole) string {
	switch role {
	case openxv1.ClusterRoleMaster:
		return "1"
	case openxv1.ClusterRoleSlave:
...
func (mc *MysqlController) newStatefulSet(m *openxv1.Mysql, in openxv1.MysqlConfig) *appsv1.StatefulSet {
	...
	sts := &appsv1.StatefulSet{
		ObjectMeta: metav1.ObjectMeta{
			//Annotations: mc.cloudConfig.Annotation(in.Service.Spec.Type, in.CloudNetworkConfig.ServiceWhiteList),
			Name:      objectName,
...
func (mc *MysqlController) syncStatefulSets(m *openxv1.Mysql, stsList []*appsv1.StatefulSet) error {
	cur := make([]*appsv1.StatefulSet, 0)
	if m.Spec.Master.Pod != nil {
		m.Spec.Master.Role = openxv1.ClusterRoleMaster
		cur = append(cur, mc.newStatefulSet(m, m.Spec.Master))
	}
	if m.Spec.Slave.Pod != nil {
		m.Spec.Slave.Role = openxv1.ClusterRoleSlave
		cur = append(cur, mc.newStatefulSet(m, m.Spec.Slave))
	}
	var errlist []error
	...
	for _, v := range add {
		if _, err := mc.stsControl.Create(v); err != nil {
			errlist = append(errlist, err)
		}
	}
	for _, v := range update {
		if _, err := mc.stsControl.Update(v); err != nil {
			errlist = append(errlist, err)
		}
	}
	for _, v := range del {
		if err := mc.stsControl.Delete(v); err != nil {
			errlist = append(errlist, err)
		}
	}
